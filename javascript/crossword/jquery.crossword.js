			...
			*/
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {		
						...
						var entry = i + 1;
						$('#' + puzz.data[i].orientation).append('<li tabindex="1" data-position="' + entry + '">' + entry + ". " + puzz.data[i].clue + '</li>'); 
					}				
					...
					for (var i=1; i <= rows; ++i) {
						tbl.push("<tr>");
							for (var x=1; x <= cols; ++x) {
								tbl.push('<td data-coords="' + x + ',' + i + '" class="content"></td>');		
							};
						tbl.push("</tr>");
					};
				...
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							light = $('td.content[data-coords="' + entries[x-1][i] + '"]');
					...
					for (var i = 0, p = entryCount; i < p; i++) {
						var entry = i + 1;
						$groupedLights = $('.entry-' + entry);
						if(!$('.entry-' + entry +':eq(0) span').length){
							$groupedLights.eq(0)
			...
			var nav = {
				...
				updateByEntry: function(e, next) {
					...
					if(e.keyCode === 9 || next){
						...
						clue = $('li[data-position=' + activePosition + ']');
			...
			var util = {
				...
				highlightClue: function() {
					var clue;				
					$('.clues-active').removeClass('clues-active');
					$('li[data-position=' + activePosition + ']').addClass('clues-active');
				...
				getActivePositionFromClassGroup: function(el){
						...
						if(classes.length > 1){
							...
							e1Ori = $('li[data-position=' + classes[0].split('-')[1] + ']').parent().prop('id');
							e2Ori = $('li[data-position=' + classes[1].split('-')[1] + ']').parent().prop('id');
